...
// Derived stores — auto-computed from other stores
export const ballColor = derived(ballHue, ($hue) => `hsl(${$hue}, 100%, 55%)`);
export const ballShadow = derived(
  ballHue,
  ($hue) => `0 0 40px hsla(${$hue}, 100%, 55%, 0.3)`
);
export const ballArea = derived(
  ballSize,
  ($size) => Math.round(Math.PI * ($size / 2) * ($size / 2))
);
